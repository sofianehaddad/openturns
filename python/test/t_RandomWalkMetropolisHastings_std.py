...
try:
    ...
    for i in range(len(sigma0s)):
        ...
        print "  expected posterior ~N( %.12g" % (w * data.computeMean()[0] + (1. - w) * mu0), ",  %.12g" % ((w * sigmay ** 2.0 / size) ** 0.5), ")"

        # try to generate a sample
        sample = sampler.getSample(50)

        print "  obtained posterior ~N( %.12g" % sample.computeMean()[0], ",  %.12g" % sample.computeStandardDeviationPerComponent()[0], ")"
