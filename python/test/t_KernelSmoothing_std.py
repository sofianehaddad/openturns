...
try:
    ...
    for i in range(kernels.getSize()):
        ...
        print "mean(smoothed)=[ %.12g" % meanSmoothed[0], ",  %.12g" % meanSmoothed[1], "] mean(exact)=[", meanExact[0], ", ", meanExact[1], "]"
    ...
    for i in range(kernels.getSize()):
        ...
        for j in range(2):
            for k in range(2):
                ...
                pointPDF = smoothed.computePDF(point)
                pointCDF = smoothed.computeCDF(point)
                print " pdf(smoothed)=  %.12g" % pointPDF, " pdf(exact)= %.12g" % distributionCollection[j].computePDF(point)
                print " cdf(smoothed)=  %.12g" % pointCDF, " cdf(exact)= %.12g" % distributionCollection[j].computeCDF(point)
