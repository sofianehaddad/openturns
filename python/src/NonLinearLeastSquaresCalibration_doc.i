...
Under standard assumptions on the observation errors (see 
:any:`code_calibration` for more details), the resulting
...
emulated using a flat :class:`~openturns.Normal` centered on the `startingPoint`
and with a variance equal to SpecFunc.Infinity.

The posterior distribution of the parameter reflects the uncertainty in the
parameter estimate arising from observation errors, under the assumed model
...
This corresponds to using the :class:`~openturns.LinearLeastSquaresCalibration` algorithm at the optimum.
However, if the key `NonLinearLeastSquaresCalibration-BootstrapSize` in the :class:`~openturns.ResourceMap`
is set to a nonzero positive integer, bootstrap resampling of the observations is used to estimate
the distribution of the calibrated parameters.
The calibrated parameter distribution is approximated based on a kernel density estimate
obtained from the bootstrap sample.
By default, a bootstrap algorithm is used.

The output (observation) error is modeled as a Normal random variable whose variance
is estimated from the calibration residuals.
This is performed whatever the value of the `NonLinearLeastSquaresCalibration-BootstrapSize`
parameter i.e., it is independent from the distribution of the parameter.
