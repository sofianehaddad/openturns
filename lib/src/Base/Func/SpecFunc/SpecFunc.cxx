                                 ...
                                 const NumericalScalar b)
{
  ...
  const NumericalScalar ratio(first / sum);
  const NumericalScalar correctionSecond(GammaCorrection(second));
  const NumericalScalar correctionSum(GammaCorrection(sum));
  const NumericalScalar log1pRatio(log1p(-ratio));
  // Case a and b large
  if (first >= 7.75) return -0.5 * log(second) + SQRT2PI + GammaCorrection(first) + correctionSecond - correctionSum + (first + 0.5) * log(ratio) + second * log1pRatio;
  // Case one of a,b large, the other small
  return LogGamma(first) + correctionSecond - correctionSum + first * (1.0 - log(sum)) + (second - 0.5) * log1pRatio;
}
