...
Scalar Kent::ComputeLogTerm(const UnsignedInteger j,
                            const Scalar kappa,
                            const Scalar logBeta,
                            const Scalar logHalfKappa)
{
  return SpecFunc::LogGamma(j + 0.5) - SpecFunc::LogGamma(j + 1.0)
         + 2.0 * j * logBeta
         - (2.0 * j + 0.5) * logHalfKappa - kappa
         + SpecFunc::LogBesselInu(kappa, 2.0 * j + 0.5);
}

Point Kent::ComputeMomentsInternal(const Scalar kappa,
                                   const Scalar beta)
{
  Point result(3);
  if (beta == 0.0)
  {
    ...
    result[0] = r1;
    result[1] = 0.0;
    result[2] = 1.0 - 2.0 * r1 / kappa;
  }
  else
  {
    const Scalar epsilon = SpecFunc::ScalarEpsilon;
    Scalar factor = 0.0;
    Scalar sumDeltaR = 0.0;
    Scalar sumJDelta = 0.0;
    Scalar sumDeltaE1 = 0.0;
    ...
    {
      Scalar delta = 0.0;
      const Scalar logTerm = ComputeLogTerm(j, kappa, logBeta, logHalfKappa);
      ...
      sumDeltaR += delta * Rj;
      sumJDelta += static_cast<Scalar>(j) * delta;
      // E[x1^2] = sum delta_j * (1 - 2*(2j+1)*R_j/kappa) / sum delta_j
      sumDeltaE1 += delta * (1.0 - 2.0 * (2.0 * static_cast<Scalar>(j) + 1.0) * Rj / kappa);
      ++j;
      if (delta < epsilon * factor) go = false;
      if (j >= maximumIteration) go = false;
    }
    result[0] = sumDeltaR / factor;
    result[1] = (2.0 / beta) * sumJDelta / factor;
    result[2] = sumDeltaE1 / factor;
  }
  return result;
}
...
Point Kent::ComputeMoments(const Scalar kappa,
                           const Scalar beta)
{
  const Point momentsInternal(ComputeMomentsInternal(kappa, beta));
  Point result(2);
  result[0] = momentsInternal[0];
  result[1] = momentsInternal[1];
  return result;
}

Point Kent::ComputeSecondMoments(const Scalar kappa,
                                 const Scalar beta)
{
  const Point momentsInternal(ComputeMomentsInternal(kappa, beta));
  const Scalar e1 = momentsInternal[2];
  const Scalar r2 = momentsInternal[1];
  Point result(3);
  result[0] = e1;
  result[1] = 0.5 * (1.0 - e1 + r2);
  result[2] = 0.5 * (1.0 - e1 - r2);
  return result;
}
...
void Kent::computeCovariance() const
{
  const Point momentsInternal(ComputeMomentsInternal(kappa_, beta_));
  // Moments in the frame (gamma_1, gamma_2, gamma_3): mean = (r1, 0, 0)
  const Scalar r1 = momentsInternal[0];
  const Scalar e1 = momentsInternal[2];
  const Scalar sigma11 = e1 - r1 * r1;
  const Scalar sigma22 = 0.5 * (1.0 - e1 + momentsInternal[1]);
  const Scalar sigma33 = 0.5 * (1.0 - e1 - momentsInternal[1]);
  SquareMatrix D(3);
  D(0, 0) = sigma11;
  D(1, 1) = sigma22;
  D(2, 2) = sigma33;
  // Cov = Gamma * diag(sigma_1^2, sigma_2^2, sigma_3^2) * Gamma^T
  const SquareMatrix tmp = gamma_ * D * gamma_.transpose();
...
void Kent::computeNormalization()
{
  ...
  while (go)
  {
    const Scalar logTerm = ComputeLogTerm(j, kappa_, logBeta, logHalfKappa);
    Scalar delta = 0.0;
    if (logTerm > SpecFunc::LogMaxScalar)
    {
      delta = SpecFunc::MaxScalar;
  ...
  logNormalization_ = std::log(2.0 * M_PI) + kappa_ + std::log(factor);
}
...
void Kent::updateSampler()
{
  ...
  Scalar upper = 3.0;
  // Bisection: f(b) = 1/b + 1/(b+a2) + 1/(b+a3) - 1 is strictly decreasing in b.
  // Stop when the interval is narrow enough for double precision.
...
Scalar Kent::computeEntropy() const
{
  const Point momentsInternal(ComputeMomentsInternal(kappa_, beta_));
  return logNormalization_ - kappa_ * momentsInternal[0] - beta_ * momentsInternal[1];
}
