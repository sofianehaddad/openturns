...
NumericalSample ComposedDistribution::getSampleParallel(const UnsignedLong size) const
{
  const UnsignedLong dimension(getDimension());
  ...
  if (hasIndependentCopula())
  {
    ...
    for (UnsignedLong i = 0; i < dimension; ++i)
    {
      ...
      for (UnsignedLong j = 0; j < size; ++j) result[j][i] = marginalSample[j][0];
    }
  ...
  NumericalSample result(size, dimension);
  const ComposedDistribution::ComputeSamplePolicy policy( copulaSample, result, distributionCollection_ );
...
NumericalSample ComposedDistribution::getSample(const UnsignedLong size) const
{
...
NumericalScalar ComposedDistribution::computeSurvivalFunction(const NumericalPoint & point) const
{
  ...
  if (hasIndependentCopula())
  {
    ...
    for (UnsignedLong i = 0; i < dimension; ++i) productSurvival *= distributionCollection_[i].computeSurvivalFunction(point[i]);
    return productSurvival;
  }
  // General case
  NumericalPoint uPoint(dimension);
  for (UnsignedLong i = 0; i < dimension; ++i) uPoint[i] = distributionCollection_[i].computeSurvivalFunction(point[i]);
...
ComposedDistribution::IsoProbabilisticTransformation ComposedDistribution::getIsoProbabilisticTransformation() const
{
  ...
  if (isElliptical())
  {
    ...
    transform.setGradientImplementation(new NatafEllipticalDistributionGradient(inverseCholesky));
    transform.setHessianImplementation(new NatafEllipticalDistributionHessian(dimension));
...
ComposedDistribution::InverseIsoProbabilisticTransformation ComposedDistribution::getInverseIsoProbabilisticTransformation() const
{
  ...
  if (isElliptical())
  {
    ...
    inverseTransform.setGradientImplementation(new InverseNatafEllipticalDistributionGradient(inverseCholesky));
    inverseTransform.setHessianImplementation(new InverseNatafEllipticalDistributionHessian(dimension));
...
ComposedDistribution::NumericalPointWithDescriptionCollection ComposedDistribution::getParametersCollection() const
{
  ...
  for (UnsignedLong marginalIndex = 0; marginalIndex < dimension; ++marginalIndex)
  {
    ...
    for (UnsignedLong i = 0; i < point.getDimension(); ++i) marginalParametersDescription[i] = (OSS() << marginalParametersDescription[i] << "_marginal_" << marginalIndex);
  ...
  if (dimension > 1)
  {
    ...
    for (UnsignedLong i = 0; i < point.getDimension(); ++i) copulaParametersDescription[i] = (OSS() << copulaParametersDescription[i] << "_copula");
...
{
  const UnsignedLong dimension(getDimension());
  for (UnsignedLong i = 0; i < dimension; ++i) if (!distributionCollection_[i].isContinuous()) return false;
  return true;
}
...
{
  const UnsignedLong dimension(getDimension());
  for (UnsignedLong i = 0; i < dimension; ++i) if (!distributionCollection_[i].isDiscrete()) return false;
  return true;
}
...
{
  const UnsignedLong dimension(getDimension());
  for (UnsignedLong i = 0; i < dimension; ++i) if (!distributionCollection_[i].isIntegral()) return false;
  return true;
}
