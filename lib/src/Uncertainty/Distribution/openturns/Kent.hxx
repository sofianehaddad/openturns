...
private:
  ...
  friend class KentFactory;

  /** Compute the first two moments r1=E[x1], r2=E[x2^2-x3^2] for given kappa,beta */
  static Point ComputeMoments(const Scalar kappa,
                              const Scalar beta);

  /** Compute the log of the j-th series term in the normalization / moment expansion */
  static Scalar ComputeLogTerm(const UnsignedInteger j,
                               const Scalar kappa,
                               const Scalar logBeta,
                               const Scalar logHalfKappa);

  /** Compute the moments r1, r2=E[x2^2-x3^2] and E[x1^2] in one pass */
  static Point ComputeMomentsInternal(const Scalar kappa,
                                      const Scalar beta);

  /** Compute the second moments E[x1^2], E[x2^2], E[x3^2] for given kappa,beta */
  static Point ComputeSecondMoments(const Scalar kappa,
                                    const Scalar beta);
