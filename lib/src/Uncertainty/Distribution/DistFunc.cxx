                                 ...
                                 const NumericalScalar p)
{
  ...
  if (n * q <= 15)
  {
    ...
    NumericalScalar t(pow(1.0 - q, static_cast<int>(n)));
    NumericalScalar s(t);
    const NumericalScalar u(RandomGenerator::Generate());
...
NumericalScalar DistFunc::rPoisson(const NumericalScalar lambda)
{
  ...
  for (;;)
  {
    ...
    const NumericalScalar k(floor(x));
...
NumericalScalar DistFunc::eZ1(const UnsignedInteger n)
{
  ...
  const NumericalScalar z0(-1.5270815222604243733 - 0.25091814704012410653 * log(static_cast<double>(n)));
  for (UnsignedInteger i = 0; i < 128; ++i)
    value += weights[i] * ((z0 + nodes[i]) * exp(-z0 * nodes[i]) * pow(pNormal(z0 + nodes[i], true), static_cast<int>(n - 1)) + (z0 - nodes[i]) * exp(z0 * nodes[i]) * pow(pNormal(z0 - nodes[i], true), static_cast<int>(n - 1)));
    ...
    const Bool tail)
{
  ...
  throw NotYetImplementedException(HERE) << "Can not give quantile value for the level " << p << ". Value is missing in table";
}
