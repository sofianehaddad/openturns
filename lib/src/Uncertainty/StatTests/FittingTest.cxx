                                   ...
                                   const UnsignedLong estimatedParameters)
{
  ...
  lastResult_.setDescription(Description(1, String(OSS() << distribution.__str__() << " vs sample " << sample.getName())));
  LOGDEBUG(OSS() << lastResult_);
  return lastResult_;
}
                                   ...
                                   const UnsignedLong estimatedParameters)
{
  ...
  const UnsignedLong nMin(ResourceMap::GetAsUnsignedLong("FittingTest-ChiSquaredMinFrequency"));
  const UnsignedLong size(sample.getSize());
  ...
  const UnsignedLong iMax(static_cast<UnsignedLong>(round(size / nMin)));
  UnsignedLong dataIndex(0);
